...
class MessageHandler {
    constructor() {
        ...
        this.responseFrequencyLimit = config.get('filtering.responseFrequencyLimit');
    ...
    async initialize() {
        try {
            ...
            this.commandHandler.personalityLoader = this.responseHandler.geminiAI.personalityLoader;
            
            this.initialized = true;
            logger.info('Message handler initialized successfully');
            console.log('✅ Message handler initialized');
        } catch (error) {
            logger.error('Failed to initialize message handler', { error: error.message });
    ...
    async handleMessage(message) {
        try {
            ...
            if (Helpers.isGroupMessage(message) && config.get('filtering.ignoreGroups')) {
            ...
            if (message.from === 'status@broadcast') {
                logger.debug('Message from status broadcast (ignored)', { from: message.from });
            ...
            if (!this.checkResponseFrequency(message.from)) {
                logger.debug('Response frequency limit reached', { from: message.from });
