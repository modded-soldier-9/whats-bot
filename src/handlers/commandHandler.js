...
*Note:* I only respond to direct messages, not group chats.`;
    ...
    async handleStatus(message, args) {
        ...
        const personality = config.get('bot.personality') === 'custom' ? 'Custom (Your Style)' : config.get('bot.personality');
...
*Note:* I remember our conversation history to provide better responses!`;
    ...
    getStats() {
        ...
        const enabledUsers = Array.from(this.userSettings.values())
            .filter(settings => settings.responsesEnabled).length;
     ...
     */
    cleanupOldSettings(maxAge = 7 * 24 * 60 * 60 * 1000) { // 7 days
        const now = Date.now();
        const toDelete = [];
