...
class WhatsAppAIBot {
    ...
    async initialize() {
        try {
            ...
            this.messageHandler.setContactInfoCallback(this.getContactInfo.bind(this));
            ...
            this.whatsappClient.onMessage(this.messageHandler.handleMessage.bind(this.messageHandler));
            console.log('✅ Message handler registered');

            logger.info('WhatsApp AI Bot initialized successfully');
            console.log('✅ WhatsApp AI Bot initialized successfully');
        } catch (error) {
            logger.error('Failed to initialize WhatsApp AI Bot', { error: error.message });
    ...
    async sendMessage(contactId, message) {
        try {
            if (!this.whatsappClient || !this.whatsappClient.isClientReady()) {
                logger.warn('WhatsApp client not ready, cannot send message', { contactId });
                return;
            }
     ...
     */
    getStats() {
        const whatsappStats = this.whatsappClient ? this.whatsappClient.getStats() : null;
        const messageStats = this.messageHandler ? this.messageHandler.getStats() : null;
