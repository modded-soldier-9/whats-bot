  ...
  loadConfig() {
    try {
      ...
      const envConfig = {
        ...
        bot: {
          personality:
            process.env.BOT_PERSONALITY || defaultConfig.bot.personality,
          responseDelay:
            parseInt(process.env.RESPONSE_DELAY) ||
            defaultConfig.bot.responseDelay,
          maxResponseLength:
            parseInt(process.env.MAX_RESPONSE_LENGTH) ||
            defaultConfig.bot.maxResponseLength,
          autoResponsesEnabled:
            process.env.AUTO_RESPONSES_ENABLED === 'true' ||
            defaultConfig.bot.autoResponsesEnabled,
        },
        whatsapp: {
          sessionTimeout:
            parseInt(process.env.SESSION_TIMEOUT) ||
            defaultConfig.whatsapp.sessionTimeout,
          qrCodeDisplay:
            process.env.QR_CODE_DISPLAY === 'true' ||
            defaultConfig.whatsapp.qrCodeDisplay,
          sessionPath: defaultConfig.whatsapp.sessionPath,
        },
        logging: {
          level: process.env.LOG_LEVEL || defaultConfig.logging.level,
          fileMaxSize:
            process.env.LOG_FILE_MAX_SIZE || defaultConfig.logging.fileMaxSize,
          fileMaxFiles:
            parseInt(process.env.LOG_FILE_MAX_FILES) ||
            defaultConfig.logging.fileMaxFiles,
          logPath: defaultConfig.logging.logPath,
        },
        commands: {
          prefix: process.env.COMMAND_PREFIX || defaultConfig.commands.prefix,
          enabled:
            process.env.ENABLE_COMMANDS === 'true' ||
            defaultConfig.commands.enabled,
          availableCommands: defaultConfig.commands.availableCommands,
        },
        filtering: {
          ignoreGroups:
            process.env.IGNORE_GROUPS === 'true' ||
            defaultConfig.filtering.ignoreGroups,
          responseFrequencyLimit:
            parseInt(process.env.RESPONSE_FREQUENCY_LIMIT) ||
            defaultConfig.filtering.responseFrequencyLimit,
