...
class PersonalityLoader {
    ...
    async initialize() {
        try {
            ...
            logger.error('Failed to initialize personality loader', { error: error.message });
            throw error;
        }
    }
    ...
    async loadPersonalities() {
        try {
            if (!await fs.pathExists(this.personalitiesPath)) {
                logger.warn('Personalities directory does not exist', { path: this.personalitiesPath });
                return;
            }
            ...
            for (const file of personalityFiles) {
                try {
                    ...
                    if (this.validatePersonality(personality)) {
                        ...
                        logger.warn('Invalid personality file', { file, errors: this.getValidationErrors(personality) });
                    }
                } catch (error) {
                    logger.error('Failed to load personality file', { file, error: error.message });
                }
            }
    ...
    validatePersonality(personality) {
        ...
        if (personality.maxResponseLength && typeof personality.maxResponseLength !== 'number') {
            return false;
        }

        if (personality.responseDelay && typeof personality.responseDelay !== 'number') {
            return false;
        }
     ...
     */
    getPersonality(name) {
        return this.personalities.get(name) || this.personalities.get(this.defaultPersonality);
    }
    ...
    getPersonalitySettings(name) {
        ...
        return {
            ...
            languageSupport: personality.languageSupport || { english: true, arabic: false },
    ...
    getLanguageInstruction(name, message) {
        ...
        if (hasArabic && personality.languageSupport.arabic) {
            return personality.languageSupport.arabicInstruction || 
